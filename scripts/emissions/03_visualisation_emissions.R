...
# Get GETS results
gets_coefs01 <- gets_results_01$isatpanel.result$mean.results
...
# same_ind <- rownames(gets_coefs01) %in% rownames(gets_coefs)
# gets_coefs <- rbind(gets_coefs, gets_coefs01[!same_ind,])
...
#   breaks_to_shade = win1_pips,
#   colorin_variable = win1_pips_temp$PIP,
...
# 
# add_break_shading_gradient(
#   breaks_to_shade = win2_pips,
#   colorin_variable = win2_pips$PIP,
#   color_variable = win2_pips$MAP,
#   ylim = c(0, 1),
#   col = c(COLORS$positive_break, COLORS$negative_break)
# )
# 
...
# break_indices_orig <- (1:(n_mod * (t_mod - 3)))[mod$coefs$omega >= PIP_THRESHOLD]
...
# 
# # Add GETS detections
# gets_indices01 <- (1:(n_mod * (t_mod - 3)))[
#   str_extract(names(mod$coefs$omega), "(?<=sis\\.).+") %in%
#     str_extract(rownames(gets_coefs01), "(?<=fesis).+")
# ]
...
#   gets_indices01_plot <- gets_indices01 + sapply(gets_indices01, function(x) {
#     sum(country_boundaries_a < x)
#   })
...
#   x = midpoints_a,
#   y = 0,
#   labels = unique(mod$data$original_data$country),
#   pos = 1,
#   xpd = TRUE,
#   cex = 0.9
# )
...
# n_total_b <- length(mod$data$y) + (n_mod - 1)
# 
...
# y_fitted_orig <- as.matrix(mod$data$X %*% mod$coefs$beta + mod$data$Z %*% mod$coefs$sis)
# if(length(mod$coefs$sigma2) > 1) {
#   sigma <- sqrt(mod$coefs$sigma2)
# } else {
...
# text(
#   x = midpoints_b,
#   y = min(y_plot, na.rm = TRUE) - 0.5,
#   labels = unique(mod$data$original_data$ISO),
#   pos = 1,
#   xpd = TRUE,
#   cex = 0.8
# )
