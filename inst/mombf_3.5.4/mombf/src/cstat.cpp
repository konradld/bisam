...
// [[Rcpp::export]]
SEXP rnlpPostCI_lm_new(SEXP niter, SEXP burnin, SEXP thinning, SEXP y, SEXP x, 
                   SEXP p, SEXP r, SEXP tau, SEXP a_phi, SEXP b_phi, SEXP prior,
                   SEXP thinit, SEXP use_thinit, SEXP phi, SEXP knownphi) {
  int n = LENGTH(y), nsave;
  SEXP ans;
  nsave = (int) (floor(INTEGER(niter)[0] - INTEGER(burnin)[0] + 0.0)/(INTEGER(thinning)[0] + 0.0));
  ans = PROTECT(Rf_allocVector(REALSXP, nsave * (INTEGER(p)[0] + 1)));
  arma::dvec thinit_vec;
  if (LOGICAL(use_thinit)[0]) {
    thinit_vec = arma::dvec(REAL(thinit), LENGTH(thinit), false, true); // no copy, use existing memory
  } else {
    thinit_vec = arma::dvec(); // empty vector if not used
  }
  rnlpPost_lm_new(REAL(ans), INTEGER(niter)[0], INTEGER(burnin)[0], INTEGER(thinning)[0], REAL(y), REAL(x), n, INTEGER(p)[0], INTEGER(r)[0], REAL(tau)[0], REAL(a_phi)[0], REAL(b_phi)[0], INTEGER(prior)[0], thinit_vec, LOGICAL(use_thinit)[0], REAL(phi)[0], LOGICAL(knownphi)[0]);
  UNPROTECT(1);
  return ans;
}

void rnlpPost_lm_new(double *ans, int niter, int burnin, int thinning, double *y, double *x, int n, int p, int r,
                 double tau, double a_phi, double b_phi, int prior, arma::dvec &thinit, bool use_thinit,
                 double phi, bool knownphi) {
  bool posdef;
...
// Lucas: Original mombf_3.5.4 version without thinit and phiinit option
// [[Rcpp::export]]
