...
SEXP modelSelectionGibbsCI(SEXP SpostModeini, SEXP SpostModeiniProb, SEXP Sknownphi, SEXP Sfamily, SEXP SpriorCoef, SEXP SpriorGroup, SEXP Sniter, SEXP Sthinning, SEXP Sburnin, SEXP Sndeltaini, SEXP Sdeltaini, SEXP Sincludevars, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Suncens, SEXP Ssumy2, SEXP Ssumy, SEXP Ssumlogyfact, SEXP Sx, SEXP Scolsumsx, SEXP ShasXtX, SEXP SXtX, SEXP SytX, SEXP Smethod, SEXP Sadjoverdisp, SEXP Shesstype, SEXP SoptimMethod, SEXP Soptim_maxit, SEXP Sthinit, SEXP Susethinit, SEXP SB, SEXP Salpha, SEXP Slambda, SEXP Sphi, SEXP Stau, SEXP Staugroup, SEXP Staualpha, SEXP Sfixatanhalpha, SEXP Sr, SEXP SpriorDelta, SEXP SprDeltap, SEXP SparprDeltap, SEXP SpriorConstr, SEXP SprConstrp, SEXP SparprConstrp, SEXP Sgroups, SEXP Sngroups, SEXP Snvaringroup, SEXP Sconstraints, SEXP Sinvconstraints, SEXP Sverbose) {
  ...
  thinit= dvector(0, mycols2+1);
  if (usethinit != 1) { // Changed by Lucas D. KONRAD from 3 to 1, see initParameters.R
    for (j=0; j<= mycols2+1; j++) { thinit[j]= 0; }
  } else {
    for (j=0; j<= INTEGER(Sp)[0]; j++) { thinit[j]= REAL(Sthinit)[j]; }
  }
...
void modelSelectionGibbs(int *postSample, double *margpp, int *postMode, double *postModeProb, double *postProb, int *prDelta, int *prConstr, int *niter, int *thinning, int *burnin, int *ndeltaini, int *deltaini, int *includevars, intptrvec *constraints, intptrvec *invconstraints, int *verbose, struct marginalPars *pars) {
  ...
  // ------------------------------------------------
  //Print all initialized variables before iteration
  if (*verbose == 1) {
    Rprintf("\n=== Variables before iteration ===\n");
    ...
    Rprintf("sumy2: %.6f\n", *(*pars).sumy2);
    Rprintf("sumy: %.6f\n", *(*pars).sumy);
    Rprintf("sumlogyfact: %.6f\n", *(*pars).sumlogyfact);
    Rprintf("method: %d\n", *(*pars).method);
    Rprintf("adjoverdisp: %d\n", *(*pars).adjoverdisp);
    Rprintf("hesstype: %d\n", *(*pars).hesstype);
    Rprintf("optimMethod: %d\n", *(*pars).optimMethod);
    Rprintf("optim_maxit: %d\n", *(*pars).optim_maxit);
    Rprintf("usethinit: %d\n", *(*pars).usethinit);
    Rprintf("B: %d\n", *(*pars).B);
    Rprintf("alpha: %.6f\n", *(*pars).alpha);
    Rprintf("lambda: %.6f\n", *(*pars).lambda);
    ...
    Rprintf("tau: %.6f\n", *(*pars).tau);
    Rprintf("taualpha: %.6f\n", *(*pars).taualpha);
    Rprintf("fixatanhalpha: %.6f\n", *(*pars).fixatanhalpha);
    Rprintf("r: %d\n", *(*pars).r);
    Rprintf("prDeltap: %.6f\n", *(*pars).prDeltap);
    Rprintf("prConstrp: %.6f\n", *(*pars).prConstrp);
    Rprintf("logscale: %d\n", *(*pars).logscale);
    Rprintf("ngroups: %d\n", *(*pars).ngroups);
    Rprintf("ngroupsconstr: %d\n", *(*pars).ngroupsconstr);
    
    if ((*pars).y != NULL && (*pars).n != NULL && *(*pars).n > 0) {
      Rprintf("y (first %d): ", (int)min_xy(5, *(*pars).n));
      for (j=0; j<(int)min_xy(5, *(*pars).n); j++) Rprintf("%.4f ", (*pars).y[j]);
      Rprintf("\n");
    }

    // if ((*pars).uncens != NULL && (*pars).n != NULL && *(*pars).n > 0) {
    //   Rprintf("uncens (first %d): ", (int)min_xy(10, *(*pars).n));
    //   // for (j=0; j<(int)min_xy(10, *(*pars).n); j++) 
    //   Rprintf("%d ", (*pars).uncens[0]);
    //   Rprintf("\n");
    // }
    
    if ((*pars).colsumsx != NULL && (*pars).p != NULL && *(*pars).p > 0) {
      Rprintf("colsumsx (first %d): ", (int)min_xy(10, *(*pars).p));
      for (j=0; j<(int)min_xy(10, *(*pars).p); j++) Rprintf("%.4f ", (*pars).colsumsx[j]);
      Rprintf("\n");
    }

    if ((*pars).ytX != NULL && (*pars).p != NULL && *(*pars).p > 0) {
      Rprintf("ytX (first %d): ", (int)min_xy(10, *(*pars).p));
      for (j=0; j<(int)min_xy(10, *(*pars).p); j++) Rprintf("%.4f ", (*pars).ytX[j]);
      Rprintf("\n");
    }

    if ((*pars).thinit != NULL && (*pars).p != NULL && *(*pars).p > 0) {
      Rprintf("thinit (first %d): ", (int)min_xy(10, *(*pars).p));
      for (j=0; j<(int)min_xy(10, *(*pars).p); j++) Rprintf("%.4f ", (*pars).thinit[j]);
      Rprintf("\n");
    }

    if ((*pars).taugroup != NULL && ngroups > 0) {
      Rprintf("taugroup (first %d groups): ", (int)min_xy(10, ngroups));
      for (j=0; j<(int)min_xy(10, ngroups); j++) Rprintf("%.4f ", (*pars).taugroup[j]);
      Rprintf("\n");
    }
    
    if ((*pars).parprDeltap != NULL) {
      Rprintf("parprDeltap (first 5): ");
      for (j=0; j<5; j++) Rprintf("%.4f ", (*pars).parprDeltap[j]);
      Rprintf("\n");
    }
    
    if ((*pars).parprConstrp != NULL) {
      Rprintf("parprConstrp (first 5): ");
      for (j=0; j<5; j++) Rprintf("%.4f ", (*pars).parprConstrp[j]);
      Rprintf("\n");
    }
    
    if ((*pars).groups != NULL && (*pars).p != NULL && *(*pars).p > 0) {
      Rprintf("groups (first %d): ", (int)min_xy(20, *(*pars).p));
      for (j=0; j<(int)min_xy(20, *(*pars).p); j++) Rprintf("%d ", (*pars).groups[j]);
      Rprintf("\n");
    }
    
    if ((*pars).isgroup != NULL && (*pars).p != NULL && *(*pars).p > 0) {
      Rprintf("isgroup (first %d): ", (int)min_xy(20, *(*pars).p));
      for (j=0; j<(int)min_xy(20, *(*pars).p); j++) Rprintf("%d ", (*pars).isgroup[j]);
      Rprintf("\n");
    }
    
    if ((*pars).nvaringroup != NULL && ngroups > 0) {
      Rprintf("nvaringroup (all %d groups): ", ngroups);
      for (j=0; j<ngroups; j++) Rprintf("%d ", (*pars).nvaringroup[j]);
      Rprintf("\n");
    }
    
    if (nconstraints != NULL && ngroups > 0) {
      Rprintf("nconstraints (first %d groups): ", (int)min_xy(10, ngroups));
      for (j=0; j<(int)min_xy(10, ngroups); j++) Rprintf("%d ", nconstraints[j]);
      Rprintf("\n");
    }
    
    if (ninvconstraints != NULL && ngroups > 0) {
      Rprintf("ninvconstraints (first %d groups): ", (int)min_xy(10, ngroups));
      for (j=0; j<(int)min_xy(10, ngroups); j++) Rprintf("%d ", ninvconstraints[j]);
      Rprintf("\n");
    }
    
    Rprintf("--- End pars structure ---\n\n");
    ...
    Rprintf("iupper: %d\n", iupper);
    Rprintf("nbvars: %d\n", nbvars);
    Rprintf("nbfamilies: %d\n", nbfamilies);
    Rprintf("ngroups: %d\n", ngroups);
    Rprintf("nsel: %d\n", nsel);
    
    // Selected variables (initial model)
    Rprintf("Initial model (sel): ");
    for (j=0; j<nsel; j++) Rprintf("%d ", sel[j]);
    if ((*family)==0) Rprintf("%d ", sel[nsel]); // baseline model
    Rprintf("\n");
    
    // Current joint probability
    Rprintf("currentJ: %.6f\n", currentJ);
    Rprintf("postModeProb: %.6f\n", *postModeProb);
    Rprintf("postProb[0]: %.6f\n", postProb[0]);
    
    // Groups information
    Rprintf("firstingroup: ");
    for (j=0; j<ngroups; j++) Rprintf("%d ", firstingroup[j]);
    Rprintf("\n");
    
    Rprintf("sample: ");
    for (j=0; j<ngroups; j++) Rprintf("%d ", sample[j]);
    Rprintf("\n");
    
    // Initial margpp values
    Rprintf("Initial margpp (first 10): ");
    for (j=0; j<(int)min_xy(10, *(*pars).p); j++) Rprintf("%.4f ", margpp[j]);
    Rprintf("\n");
    
    // postMode
    Rprintf("postMode (variables in initial model): ");
    for (j=0; j<*(*pars).p; j++) if (postMode[j]==1) Rprintf("%d ", j);
    Rprintf("\n");
    
    Rprintf("=== End of initial variables ===\n\n");
  }
  // ------------------------------------------------
  ...
  for (i=ilow, itercount=0; i< iupper; i++, itercount++) {
    j= jgroup= 0;
    if (*verbose == 1) {
      Rprintf("----- Within Loop iteration %d -----\n", i);
    }
    while (j< *(*pars).p) {
      ...
      if (sample[jgroup]>0) { //if jgroup should be sampled
        ...
        if (*verbose == 1) {
          Rprintf("newJ: %.6f", newJ[0]);
        }

      //   if ((nonbinary) && sample[jgroup]==2) { //non-binary update // L: irellevant for us
      // 
