...
setMethod("rnlp", signature(y='missing',x='missing',m='missing',V='missing',msfit='msfit',outcometype='missing',family='missing'), function(y, x, m, V, msfit, outcometype, family, priorCoef, priorGroup, priorVar, niter=10^3, burnin=round(niter/10), thinning=1, pp='norm') { #, thinit=NULL,use_thinit=FALSE,phiinit=NULL,use_phiinit=NULL) {
...
setMethod("rnlp", signature(y='ANY',x='matrix',m='missing',V='missing',msfit='msfit',outcometype='missing',family='missing'), function(y, x, m, V, msfit, outcometype, family, priorCoef, priorGroup, priorVar, isgroup, niter=10^3, burnin=round(niter/10), thinning=1, pp='norm') { #, thinit=NULL,use_thinit=FALSE,phiinit=NULL,use_phiinit=NULL) {
...
setMethod("rnlp", signature(y='ANY',x='matrix',m='missing',V='missing',msfit='missing',outcometype='missing',family='missing'), function(y, x, m, V, msfit, outcometype, family, priorCoef, priorGroup, priorVar, isgroup, niter=10^3, burnin=round(niter/10), thinning=1, pp='norm') { #, thinit=NULL,use_thinit=FALSE,phiinit=NULL,use_phiinit=NULL) {
    rnlp(y=y,x=x,family='Continuous')
}
)


setMethod("rnlp", signature(y='ANY',x='matrix',m='missing',V='missing',msfit='missing',outcometype='character',family='character'), function(y, x, m, V, msfit, outcometype, family, priorCoef, priorGroup, priorVar, isgroup, niter=10^3, burnin=round(niter/10), thinning=1, pp='norm') { #, thinit=NULL,use_thinit=FALSE,phiinit=NULL,use_phiinit=NULL) {
  if ((outcometype== 'Continuous') && (family== 'normal')) {  ##Linear model
    ...
    ans <- rnlp_new(y=y,x=x,priorCoef=priorCoef,priorGroup=priorGroup,priorVar=priorVar,isgroup=isgroup,niter=niter,burnin=burnin,thinning=thinning,thinit=NULL,use_thinit=FALSE,phi=NULL,knownphi=FALSE) # L: try new rnlp_new() with th_init and known phi for plugin-setup
...
rnlp_new <- function(y, x, priorCoef, priorGroup, priorVar, isgroup, niter=10^3, burnin=round(niter/10), thinning=1, thinit=NULL, use_thinit=FALSE, phi=NULL, knownphi=FALSE) {
  ...
  if ((priorCoef@priorDistr %in% c('pMOM','peMOM','piMOM')) && (priorCoef@priorDistr == priorGroup@priorDistr)) {
    ...
    if (p==0) {
      ...
      if(is.null(phi)) {
        if(!knownphi) {
          phi <- -1 # L: if true set to -1 for clarity but never used again in the C++ code
        } else {
          stop("Missing phi value. Either select knownphi == FALSE or select a phi value")
        }
      }
      ans <- rnlpPostCI_lm_new(as.integer(niter),as.integer(burnin),as.integer(thinning),as.double(y),as.double(x),as.integer(p),as.integer(r),tau,a_phi,b_phi,prior,thinit,use_thinit,phi,knownphi)
...
setMethod("rnlp", signature(y='missing',x='missing',m='numeric',V='matrix',msfit='missing',outcometype='missing',family='missing'), function(y, x, m, V, msfit, outcometype, family, priorCoef, priorGroup, priorVar, isgroup, niter=10^3, burnin=round(niter/10), thinning=1, pp='norm') { #, thinit=NULL,use_thinit=FALSE,phiinit=NULL,use_phiinit=NULL) {
  if (missing(priorGroup)) priorGroup= priorCoef
  p <- ncol(V)
