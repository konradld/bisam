...
coef.msfit <- function(object,niter=10^4,burnin=round(niter/10),meanonly=FALSE,...) { # changed by L.D.K.
    hasPostSampling(object)
    th= rnlp(msfit=object,niter=10,burnin=burnin)
...
# - postProb: unnormalized posterior prob of each visited model (log scale)
  ...
  tmp= codeGroupsAndConstraints(p=p,groups=groups,constraints=constraints) # Lucas: delete the constraint argument for line-by-line execution, then plug in again ;)
  ...
  thinit= getthinit(y=y, x=xstd, family=family, initpar=initpar, enumerate=enumerate) # L: in initParameters.R run glmBIC, initParameters, getthinit
  ...
  priorConstraints <- defaultpriorConstraints(priorDelta, priorConstraints) # Lucas: delete the priorConstraints argument for line-by-line execution, then plug in again ;)
    ...
    if(verbose){
      print(
        list(
          ...
          taugroup = taugroup,
          taualpha = taualpha,
          fixatanhalpha = fixatanhalpha,
          r = r,
          prDelta = prDelta,
          prDeltap = prDeltap,
          parprDeltap = parprDeltap,
          prConstr = prConstr,
          prConstrp = prConstrp,
          parprConstrp = parprConstrp
          # groups = groups,
          # ngroups = ngroups,
          # nvaringroup = nvaringroup,
          # constraints = constraints,
          # invconstraints = invconstraints,
